...
const selectBuscador = (datos) =>
{
	...
	function consultarWebApi(consulta = {}) 
	{
		...
		var parametrosParaEnviar = {};  // Se realiza un json separado ya que los valores de la consulta pueden ir cambiando
		
		for (var key in consulta)  // Se recorre el json con los parametros
		{
			if (typeof consulta[key] === "function")   // Si dentro del elemento recorrido viene una funcion, obtiene el resultado de esa funcion y lo coloca 
			{          
				parametrosParaEnviar[key] = consulta[key]();  
			}   
			else {
				parametrosParaEnviar[key] = consulta[key]; 
			}
		...
		//console.log(parametrosParaEnviar);
		
		//=====================================>>>>>

		var parametros = "?" + Object.entries(parametrosParaEnviar).map(c=> c.join("=")).join("&"); 
